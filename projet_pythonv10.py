...
def printSalle():
    ...
    # Affichage des salles
    header_list = ['Bâtiment', 'Numéro', 'Capacité', 'Type']
    width_list = [8, 8, 10, 11]
    length = 51
    id_list = printTable(listSalle, header_list, width_list, length)
...
def addSalle():
    ...
    foundSalle = run('SELECT * FROM salle where numsalle=%s', (numSalle, capacite))
...
def deleteEtudiant():
    ...
    deleteEtudiantCompose(idEtudiant)
...
def updateAssociation():
    printAssociation()
    nom = input('\n\nNom de l\'association à modifier : ')
    ...
    sql2='SELECT idEtudiant FROM compose WHERE nomAsso=%s AND statut=%s'
    idPresident = run(sql2,(nom, 'Président'))
    sql3='SELECT idEtudiant FROM compose WHERE nomAsso=%s AND statut=%s'
    idTresorier = run(sql3,(nom, 'Trésorier'))
    modif=True
    while True:
        ...
        elif choix == '5': # Président
            idPresident = chooseEtudiant()
            run('DELETE FROM compose WHERE nomAsso=%s AND statut=%s',(nom, 'Président'))
            run('UPDATE compose SET statut=%s WHERE nomAsso=%s AND idEtudiant=%s', ('Président', nom, idPresident))
            break

        elif choix == '6': # Trésorier
            idTresorier =chooseEtudiant()
            run('DELETE FROM compose WHERE nomAsso=%s AND statut=%s',(nom, 'Trésorier'))
            run('UPDATE compose SET statut=%s WHERE nomAsso=%s AND idEtudiant=%s', ('Trésorier', nom, idTresorier))
            break
...
def updateSpectacle(): 
    ...
    if categorie == 'Concert':
        ...
        while True:
            ...
            if choix == '1':
                organisateur = input('Nouvel organisateur : ')
                break
...
def updateSalle():
    ...
    while True:
        ...
        if choix == '1':
            capacite = input('Nouvelle capacité : ')
            break

        elif choix == '2':
            libelleType = input('Nouveau libellé du type de la salle : ')
            break
    ...
    if modif:
        run('UPDATE salle SET capacite=%s',(capacite,))
        run('UPDATE typeSalle SET libelleType=%s',(libelleType,))
...
def updateReservation():
    numBat, numSalle, date = chooseReservation()
    sql1 = 'SELECT nom FROM reservation WHERE numBat=%s AND numSalle=%s AND date=%s'
    association = run(sql1,(numBat, numSalle, date))
    ...
    while True:
        ...
        if choix == '1':
            numBat = input('Nouveau numéro de bâtiment: ')
            break

        elif choix == '2':
            numSalle = input('Nouveau numéro de salle : ')
            break

        elif choix == '3':
            date = inputDate('Nouvelle date de réservation (DD/MM/YYYY) : ')
            break

        elif choix == '4':
            association = chooseAssociation()
            run('DELETE FROM reservation WHERE numBat=%s AND numSalle=%s AND date=%s',(numBat, numSalle, date))
            break

        elif choix == '5':
            break

    run('UPDATE reservation SET nom=%s, numBat=%s, numSalle=%s, date=TO_DATE(%s,\'DD/MM/YYYY\')',(association, numBat, numSalle, str(date)))
        ...
        if choix == '1': # utilisateur
            ...
            while True:
                ...
                if choixSalle == '1':
                    ...
                    os.system('cls' if os.name == 'nt' else 'clear')
                    printTitle('Modification d\'une réservation')
                    updateReservation()
                    os.system('cls' if os.name == 'nt' else 'clear')
                    break
                elif choixSalle == '6':
